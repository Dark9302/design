...
@section('content')
    <div class="page-container">
        <form class="form form-horizontal" id="article-add" enctype="multipart/form-data" method="post">
            <input type="hidden" value="{{$pic->id}}" id="id" name="id">
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>大标题：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" class="input-text" value="{{$pic->title1}}" placeholder="" name="title1" id="title1">
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>小标题：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" class="input-text" value="{{$pic->title2}}" placeholder="" name="title2" id="title2">
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>连接地址：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" class="input-text" value="{{$pic->url}}" placeholder="" name="url" id="url">
                </div>
            </div>
            <div class="row cl">
    ...
    <script>
        ...
        $("#article-add").validate({
            ...
            submitHandler:function(form){
                ...
                var url = $("#url").val();
                $(form).ajaxSubmit({
                    url:"{{url('admin/doEditPic')}}",
                    type:"post",
                    data:{
                        'id':id,'title1':title1,'title2':title2,'url':url
                    },
                    headers: {
